...
class DialogueSession:
	...
	def reset(self, **kwargs):
		#_print_debug("DialogueSession: reset session {} after {} ms".format(self._active_rhasspy_session, self.elasped_time))
	...
	def reset_rhasspy_session(self, **kwargs):
		...
		#print(self._active_rhasspy_session)
		#rospy.logdebug('DialogueSession: reset rhasspy session %s after {} ms',str(self._active_rhasspy_session))
		self._active_rhasspy_session = None
		self._site_rhasspy_id = None	
...
class DialogueManager(ManagerInterface):
	...
	def init_ros_topics(self):
		...
		self.pub_intent_not_rec = rospy.Publisher(ROS_SERVICE_PREFIX + 'intent/not_recognized', String, queue_size=10)
	...
	def execute_utter_ros_service(self, request):
		# print('execute_utter_ros_service',request)
		#context_id = request.context_id
		#print('What??', context_id)
		#response = UtterServiceResponse()
		#response.success = self.direct_interaction(type='notification', text=request.text, canBeEnqueued=request.canbeenqued)
		#print('execute_utter_ros_service',response)
		return self.execute_interaction_ros_service(request, utter_only=True)
	
	def execute_interaction_ros_service(self, request, utter_only=False):
		...
		if context_id is not None and len(context_id):
			...
			retval = self.direct_interaction(type=interaction_type, text=request.text, canBeEnqueued=request.canbeenqued, customData=context_id)
			...
			response.success = retval
		rospy.logdebug("executed inteaction with result: {}".format(response))
		return response
	...
	def execute_utter_actionlib_callback(self, goal_handle):
		...
		self.direct_interaction(type='notification', text=sentence, canBeEnqueued=False)
		#elf.utter(text=sentence)
	...
	def direct_interaction(self, type='action', text='something to infer', canBeEnqueued=True, customData=None):
	...
	def handle_intents(self, specific_topic, client, userdata, message):
		...
		if self._USE_INTENT_SERVICE:
			...
			try:
				...
				rospy.logdebug("called service "+ service_name +" with response : " + str(resp))
			except Exception as e:
				rospy.logwarn("Error calling  {}: {}".format(service_name, e))
		else:
			try:
				msg = IntentMessage(context_id=context_id, intent_name=ros_intent_name, pay_load=ros_pay_load, confidence=ros_confidence)
		...
		if specific_topic == 'sessionStarted':
			...
			try:		
				...
				if termination_reason == 'intentNotRecognized':
					...
					print("+-+-+-+-+-+-REASON: UNMANAGED ", termination_reason)
					self._session.close_session(rh_session_id=payload['sessionId'], rh_site_id=payload['siteId'], session_data=payload['customData'])
			except MachineError as e:
				rospy.logwarn('+-+-+-+-+-+-handle_dialogue:'+str(specific_topic)+': ERROR %s', str(e))
			except Exception as e:
	...
	def handle_nlu(self, specific_topic, client, userdata, message):
		...
		if specific_topic == 'intentParsed':
			...
			try:
				_print_topic_data('handle_nlu', specific_topic, payload, parameters) # + [payload['customData']])
			...
			_print_topic_data('handle_nlum not supported', specific_topic, payload, None)
	...
	def handle_tts(self, specific_topic, client, userdata, message):
		payload = json.loads(message.payload)
		#_print_topic_data('handle_tts', specific_topic, payload, None)
